...
release=Centos
basename=$(pwd)
zabbixsrc=$basename/zabbix-6.0.13
...
function InitSystem() {
        ...
        if ! id zabbix &> /dev/null; then
                ...
                useradd --system -g zabbix zabbix || true
        fi
...
function ZbxInstall() {
        ...
        wget -c https://cdn.zabbix.com/zabbix/sources/stable/6.0/zabbix-6.0.13.tar.gz -o /dev/null -O - | tar -xz
